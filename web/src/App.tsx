...
export function App() {
  ...
  const [metrics, setMetrics] = useState({
    inVehicle: false, speed: 0, yaw: 0, x: 0, z: 0, drawCalls: 0, frame: 0,
    health: 100, heat: 0, treasury: 2500, day: 1, clock: 9, respect: 10, vtype: 0, district: 0,
  });
    ...
    <div className="app">
      <header className="header">
        ...
        <span className="status">
          ...
          {status === "ready" ? `ready (ABI v${version}) · WebGL2` : status === "loading" ? "loading…" : status === "error" ? "error" : "idle"}
        </span>
      </header>
      ...
      <aside className="panel">
        <section>
          ...
          <div className="metrics">
            <div>Cash <strong>${metrics.treasury}</strong></div>
            <div>Respect <strong>{metrics.respect}</strong></div>
            <div>Heat <strong>{metrics.heat}</strong></div>
            <div>HP <strong>{metrics.health}</strong></div>
            <div>Day <strong>{metrics.day}</strong></div>
            <div>Time <strong>{String(hh).padStart(2, "0")}:{String(mm).padStart(2, "0")}</strong></div>
          </div>
        </section>
        <section>
          <h2>Session</h2>
          <div className="metrics">
            ...
            <div>In vehicle <strong>{metrics.inVehicle ? "yes" : "no"}</strong></div>
            <div>Speed <strong>{metrics.speed.toFixed(1)}</strong></div>
            <div>Car <strong>{VTYPES[metrics.vtype] ?? "?"}</strong></div>
          </div>
          <button onClick={init} disabled={status === "loading"}>Reload</button>
          <div style={{ display: "flex", gap: "0.4rem", marginTop: "0.5rem" }}>
            <button onClick={() => {
              const m = moduleRef.current; if (!m) return;
              localStorage.setItem("crucible_save_v1", JSON.stringify({
                ...
                x: m.crucible_metric_player_x(), z: m.crucible_metric_player_z(), yaw: m.crucible_metric_yaw(),
              }));
            }}>Save</button>
            <button onClick={() => {
              const m = moduleRef.current; if (!m) return;
              const raw = localStorage.getItem("crucible_save_v1"); if (!raw) return;
              try {
                const d = JSON.parse(raw);
                m.crucible_load_state?.(d.district ?? 0, d.on_foot ?? 1, d.hp ?? 100, d.heat ?? 0, d.treasury ?? 2500, d.respect ?? 10, d.day ?? 1, d.clock ?? 9, d.x ?? 10, d.z ?? 20, d.yaw ?? 0);
              } catch {}
            }}>Load</button>
          </div>
        </section>
        <section>
          ...
          <div className="metrics" style={{ fontSize: "0.8rem" }}>
            WASD walk/drive · E interact/travel · F cycle car · H heal · Shift handbrake
            <br />Orange gates travel districts
          </div>
        </section>
      </aside>
    </div>
