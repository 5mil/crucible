...
export function createDemoModule(): CrucibleModule {
  ...
  function loadDistrict(id: District) {
    district = id;
    const fps = [[16,12,7,12,6],[16,24,6,9,5],[16,34,8,14,7],[28,12,5,8,5],[28,22,7,11,6],[28,34,6,10,5],[4,12,6,10,5],[4,24,5,7,4],[4,34,7,13,6],[-8,14,6,9,5],[-8,26,8,12,7],[-8,38,5,8,5],[40,16,6,11,5],[40,28,7,9,6],[-20,18,5,10,5],[-20,30,6,8,5],[10,0,8,6,6],[24,0,5,7,4],[-4,0,6,9,5],[36,4,5,8,4],[16,46,7,10,6],[28,46,5,8,5],[4,46,6,11,5],[-8,48,7,9,6]];
    const colors = [[90,70,60],[70,75,90],[100,85,70],[80,65,70]];
    buildings = fps.map((f, i) => {
      const c = colors[i % 4];
      return { x: f[0], z: f[1], w: f[2], h: f[3], d: f[4], r: c[0]/255, g: c[1]/255, b: c[2]/255 };
    });
    peds = Array.from({ length: 16 }, (_, i) => {
      const a = i * 0.7;
      return { x: 10 + Math.sin(a) * 18, z: 20 + Math.cos(a) * 14, yaw: a, speed: 1.2, phase: a };
    });
    traffic = Array.from({ length: 8 }, (_, i) => {
      const side = i % 2 === 0 ? 1 : -1;
      return { x: -30 + i * 12, z: 20 + side * 3.5, yaw: side > 0 ? 0 : Math.PI, speed: 6 + (i % 3), r: 0.3, g: 0.3, b: 0.4 };
    });
    vehicles = [
  ...
  const mod: CrucibleModule = {
    ...
    crucible_cam_px: () => bossX - Math.cos(bossYaw) * (onFoot ? 14 : 16),
    crucible_cam_py: () => (onFoot ? 10 : 12),
    crucible_cam_pz: () => bossZ - Math.sin(bossYaw) * (onFoot ? 14 : 16),
    crucible_cam_tx: () => bossX, crucible_cam_ty: () => 1, crucible_cam_tz: () => bossZ,
